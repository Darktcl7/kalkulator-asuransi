...
const PendidikanCalculatorV2 = () => {
  ...
  const handleReveal = () => {
    ...
    const message = `Halo Amalia, perkenalkan saya ${formData.name} (${formData.email}). Saya baru saja menggunakan Kalkulator Dana Pendidikan di website Anda. Menurut perhitungan, saya membutuhkan estimasi dana sebesar ${formatRp(results.totalFuture)}. Saya ingin berdiskusi lebih lanjut mengenai persiapan ini.`;
  ...
  const renderCopywriting = () => {
    ...
    return (
      <div className="bg-black text-white p-6 md:p-8 rounded-2xl mb-8 shadow-2xl relative overflow-hidden">
        ...
        <p className="text-xl md:text-2xl font-black mb-4 relative z-10 leading-tight">{scenario.title.replace('{years}', yearsUntilStart)}</p>
          ...
          <div className="space-y-8">
            <div className="group">
              <label className="block text-xs font-black text-gray-400 uppercase tracking-wider mb-2 group-focus-within:text-pru-maroon transition-colors">
                Estimasi Biaya Kuliah Saat Ini
              </label>
              <div className="flex items-center border-b-2 border-gray-200 group-focus-within:border-pru-maroon transition-colors pb-2">
                ...
                <input
                  type="text" inputMode="numeric" value={kuliahCost} onChange={handleCostChange}
            ...
            <div className="group">
              <label className="block text-xs font-black text-gray-400 uppercase tracking-wider mb-2 group-focus-within:text-pru-maroon transition-colors">
                Usia Anak Anda Saat Ini
              </label>
              <div className="flex items-center border-b-2 border-gray-200 group-focus-within:border-pru-maroon transition-colors pb-2">
                <input
                  type="number" value={childAge} onChange={(e) => setChildAge(e.target.value)} min="0" max="17"
                  className="w-full text-3xl md:text-4xl font-black text-black outline-none bg-transparent placeholder-gray-200"
                  placeholder="0"
                />
                <span className="text-xl font-bold text-gray-400 ml-3">Tahun</span>
              </div>
              <p className="text-gray-400 text-xs mt-2 font-medium">Isi 0 jika anak belum lahir.</p>
            </div>
          </div>
        </div>
          ...
          <div className="animate-in slide-in-from-bottom-8 fade-in duration-700">
            ...
            <div className="bg-pru-maroon text-white rounded-[2rem] shadow-2xl p-8 md:p-12 text-center relative overflow-hidden mb-8">
              ...
              <div className="relative z-10">
                <p className="text-sm font-bold uppercase tracking-widest text-red-200 mb-4">Total Biaya Kuliah {results.yearsUntilStart} Tahun Lagi</p>
                ...
                <div className="inline-block bg-black/20 backdrop-blur-sm px-6 py-2 rounded-full border border-white/10">
                  <p className="text-xs md:text-sm font-medium">Tercapai di usia 18 tahun</p>
                </div>
              </div>
            </div>
            ...
            {isAgent === 'yes' || isRevealed ? (
              ...
              <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-6 md:p-10 border border-gray-100 animate-in fade-in duration-500">
                <h3 className="text-xl font-black text-black mb-6">Rincian Proyeksi Pendidikan</h3>
                
                <div className="grid gap-4 md:grid-cols-2 mb-8">
                  <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                    <p className="text-xs text-gray-500 font-bold uppercase mb-1">Biaya Hari Ini</p>
                    <p className="text-2xl font-black text-black">{formatRp(results.totalToday)}</p>
                  </div>
                  <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                    <p className="text-xs text-gray-500 font-bold uppercase mb-1">Waktu Persiapan</p>
                    <p className="text-2xl font-black text-pru-maroon">{results.yearsUntilStart} Tahun</p>
                  </div>
                </div>
                ...
                {isRevealed && isAgent === 'no' && (
                  <div className="bg-green-50 border-l-4 border-green-500 p-6 rounded-r-2xl mb-6">
                    ...
                    <p className="text-sm text-green-700">Amalia PRU akan membantu Anda menyusun asuransi pendidikan anak mulai hari ini. Silakan lanjutkan obrolan di WhatsApp.</p>
                  </div>
                )}
                <p className="text-xs text-gray-400 text-center mt-4">
                  *Asumsi inflasi pendidikan rata-rata 10% per tahun. Angka ini adalah proyeksi matematis.
                </p>
              </div>
            ) : (
              // LOCKED (LEAD GATE)
              <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.08)] p-6 md:p-10 border border-gray-100">
                  <div className="text-center mb-8">
                    <h3 className="text-2xl font-black text-black mb-2">Buka Kunci Rincian</h3>
                    <p className="text-gray-500 text-sm">Jangan biarkan inflasi pendidikan menghancurkan masa depan anak. Lengkapi data di bawah ini.</p>
                  </div>
                  ...
                  <label className="flex items-start gap-4 cursor-pointer mb-8 p-5 bg-gray-50 rounded-2xl border-2 border-gray-200 hover:border-gray-300 transition-all">
                    ...
                    <span className="text-sm text-gray-700 leading-relaxed font-medium">
                      Saya setuju terhubung dengan Amalia PRU via WhatsApp untuk mendapatkan konsultasi persiapan dana kuliah.
                    </span>
                  </label>
...
export default PendidikanCalculatorV2;
